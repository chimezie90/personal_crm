...
/**
 * Instagram DM connector for importing message exports
 *
 * Instagram messages are exported via:
...
export class InstagramConnector extends BaseConnector {
  ...
  async init(): Promise<Result<void>> {
    ...
    if (this.exportPath) {
      ...
      if (!existsSync(this.exportPath)) {
        this.setState("error");
        return err(
          new Error(`Instagram export directory not found: ${this.exportPath}`)
        );
      }
    }
  ...
  /**
   * Fetch messages from Instagram export
   */
  async *fetchMessages(since?: Date): AsyncGenerator<RawMessage> {
    ...
    let count = 0;
    for await (const message of parseInstagramExport(
      this.exportPath,
      this.parseOptions
    )) {
  ...
  /**
   * Fetch contacts from Instagram export
   */
  async *fetchContacts(): AsyncGenerator<RawContact> {
    ...
    let count = 0;
    for await (const contact of extractInstagramContacts(
      this.exportPath,
      this.parseOptions
    )) {
      yield contact;
...
registerConnector("instagram", (config, events) => {
  return new InstagramConnector(config, events);
});
